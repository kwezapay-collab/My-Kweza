(function () {
  ...
  const initPageEnterTransition = () => {
    ...
    // Double requestAnimationFrame to ensure the browser has painted the hidden/moved state
    // before we apply the transition class.
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        document.body.classList.add(PAGE_TRANSITION_READY_CLASS);
        // Force layout to ensure any position: fixed elements recalculate their container
  ...
  const setupBottomNav = () => {
    ...
    // Don't show on login page or dedicated menu page
    if (path === '/' || path.endsWith('/index.html') || path.endsWith('/menu.html')) return;
    ...
    nav.className = 'bottom-nav animate-fade';
    ...
    nav.innerHTML = items.map(item => {
      const isActive = path.endsWith(item.href) || (item.href === '/dashboard.html' && path === '/dashboard');
      const isMore = item.href === '#more';
      return `
        <a href="${isMore ? 'javascript:void(0)' : item.href}" class="bottom-nav-item ${isActive ? 'active' : ''}" ${isMore ? 'id="bottomNavMoreBtn"' : ''}>
  ...
  const setupDashboardNavMenu = () => {
    ...
    // Avoid double injection for the side menu
    if (document.querySelector('.nav-hamburger-wrap')) return;

    const isDedicatedMenuPage = window.location.pathname.toLowerCase().endsWith(MENU_PAGE_PATH);
    const menuHost = document.body || document.documentElement; // Always append to body or root for global sidebar behavior

    const isActionMount = false; // We no longer use mount points for the launcher
    ...
    menuWrap.innerHTML = `
      ...
      <div class="nav-hamburger-menu" role="menu" aria-label="Navigation menu">
        ...
        <div class="nav-menu-items">
          ...
          <a id="navMenuPayoutsLink" class="nav-menu-link ${window.location.pathname.endsWith('/payouts-history.html') ? 'active' : ''}" role="menuitem" href="/payouts-history.html">
    ...
    const closeMenu = ({ preserveHash = false } = {}) => {
      ...
      document.querySelector('#bottomNavMoreBtn')?.classList.remove('active');
    ...
    const openMenu = () => {
      ...
      document.querySelector('#bottomNavMoreBtn')?.classList.add('active');
      setStoredMenuHomePath(menuHomePath);
      setMenuHashOnCurrentEntry();
    };

    // Global toggle for the side menu via bottom nav
    const attachBottomNavTrigger = () => {
      const moreBtn = document.querySelector('#bottomNavMoreBtn');
      if (moreBtn) {
        // Use addEventListener for better reliability than .onclick
        moreBtn.onclick = null; // Clear any existing onclick
        ...
        // Retry if not yet in DOM (safety)
        setTimeout(attachBottomNavTrigger, 100);
      }
    };
    attachBottomNavTrigger();
    ...
    document.addEventListener('click', (event) => {
      // Don't close if clicking the bottom nav more button (it handles its own toggle)
      if (event.target.closest('#bottomNavMoreBtn')) return;
      closeMenu();
    });
